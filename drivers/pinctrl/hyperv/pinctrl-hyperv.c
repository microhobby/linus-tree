...
static int hyperv_gpio_direction_input(struct gpio_chip *chip, unsigned offset)
{
	int *p = 0;

	printk(KERN_INFO "%s: config my virtual bank %d line %d to input\n",
	...
	if (offset == 22) {
		printk(KERN_INFO "WSL CONF this is a BUG\n");
		BUG();
	}

	if (offset == 7) {
		printk(KERN_INFO "WSL CONF this is a oops\n");
		printk("%d\n", *p);
	}
...
{
	printk(KERN_INFO "%s: config my virtual bank %d line %d to output %d\n",
		__func__, chip->gpiodev->id, offset, value);

	return 0;
}

static void hyperv_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	int *p = 0;
...
static int hyperv_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	printk(KERN_INFO "%s: get my virtual bank %d line %d \n",
		__func__, chip->gpiodev->id, offset);
...
MODULE_DESCRIPTION("Hyper-V WSL 2 pinctrl/GPIO driver");
